...
public class AiAssistantServiceImpl implements AiAssistantService {
    ...
    public List<Map<String, Object>> getSmartRecommendations(Long userId, String preference, Long shopId) {
        ...
        try {
            ...
            //根据订单id获取所有订单详情
            List<OrderDetail> byOrderIds = orderDetailMapper.getByOrderIds(orderIds);
            //获取所有的菜品id
            List<Long> orderDishIds = byOrderIds.stream().map(OrderDetail::getDishId).toList();
            //用户购买最多的菜品
            List<Dish> favoriteDishes = dishMapper.getDishByIds(orderDishIds);
            //用户购买最多的就是喜欢的菜品
            //根据热度排序
            ...
            //浏览最多的菜品
            List<Dish> itemViewDishes = dishMapper.getDishByIds(itemViewDishIds);
    ...
    private List<Map<String, Object>> getAiRecommendations(List<Dish> allDishes, String preference) {
        try {
            ...
            String userPrompt = String.format(
                    "用户偏好：%s\n\n可选菜品列表：\n%s\n\n请从上述菜品和套餐中推荐10道最符合用户偏好的菜品，至少返回一个套餐，并以JSON格式返回，格式如下：\n" +
                            "[\n" +
                            "  {\n" +
                            "    \"id\": 菜品ID,\n" +
                            "    \"name\": \"菜品名称\",\n" +
